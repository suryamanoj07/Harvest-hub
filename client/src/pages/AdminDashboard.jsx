...
const AdminDashboard = () => {
  ...
  const fetchUserGrowthData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/admin/user-growth-data"
      );
      if (response.data.success) {
        setUserGrowthData({
  ...
  const fetchRevenueData = async (period) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/admin/revenue-data?period=${period}`
      );
      if (response.data.success) {
        setRevenueData({
          labels: response.data.data.labels,
  ...
  const fetchTopSellers = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/admin/top-sellers"
      );
      if (response.data.success) {
  ...
  const fetchTopSellingProducts = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/product/top-selling-products"
      );
      if (response.data.success) {
  ...
  return (
    <div>
      <Sidebar />
      <div className="admin-dashboard ml-48">
        <div className="dashboard-container">
          <h1 className="text-center text-2xl text-blue-500 font-bold mb-8">Admin Dashboard</h1>
            ...
            <div className="top-sellers-list">
              {topSellers.map((seller, index) => (
                <div key={index} className="top-seller">
                  <p>
                    <strong>Email:</strong> {seller._id}
                  </p>
                  <p>
                    <strong>Name:</strong> {seller.name}
                  </p>
                  <p>
                    <strong>Stock:</strong> {seller.stock}
                  </p>
                  <p>
                    <strong>Revenue:</strong> Rs {seller.revenue}/-
                  </p>
                </div>
              ))}
            </div>
