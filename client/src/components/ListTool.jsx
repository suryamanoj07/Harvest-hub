...
const ListTool = () => {
  const url = 'http://localhost:3000/api/tool/list';
  ...
  const fetchList = async () => {
    ...
    if (response.data.success) {
      ...
      toast.success('Tool list fetched successfully');
    } else {
      toast.error('Error fetching tool list');
    }
  };

  const removeTool = async (id) => {
    const response = await axios.post(`http://localhost:3000/api/tool/delete/${id}`);
    await fetchList();
    if (response.data.success) {
  ...
  const updateStock = async (id) => {
    const response = await axios.put(
      `http://localhost:3000/api/tool/update/${id}`,
